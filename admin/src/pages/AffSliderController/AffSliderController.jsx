...
const AffSliderController = () => {
  ...
  const { data: sliders = [], isLoading } = useQuery({
    queryKey: ["admin-aff-sliders"],
    queryFn: async () => {
      const res = await api.get("/api/aff-sliders/admin/all");
  ...
  const selectedFileName = useMemo(() => form.image?.name || "", [form.image]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEdit && !form.image) {
      toast.error("Affiliate slider image is required");
      return;
    }
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/api/aff-sliders/${editingId}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Affiliate slider updated successfully");
      } else {
        await api.post("/api/aff-sliders", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Affiliate slider created successfully");
      }

      resetForm();
      queryClient.invalidateQueries({ queryKey: ["admin-aff-sliders"] });
      queryClient.invalidateQueries({ queryKey: ["aff-client-sliders"] });
    } catch (error) {
      console.error(error);
      toast.error(
        error?.response?.data?.message || "Failed to save affiliate slider",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm(
      "Are you sure you want to delete this affiliate slider?",
    );
    if (!ok) return;

    try {
      await api.delete(`/api/aff-sliders/${id}`);
      toast.success("Affiliate slider deleted successfully");
      ...
      console.error(error);
      toast.error(
        error?.response?.data?.message || "Failed to delete affiliate slider",
      );
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-blue-100/80 mt-1">
                  Add, update and delete affiliate site sliders
                </p>
              </div>
            </div>
          ...
          >
            <div className="flex items-center justify-between gap-4 mb-6">
              <h2 className="text-xl font-bold text-white">
                {isEdit ? "Update Affiliate Slider" : "Add Affiliate Slider"}
              </h2>
              ...
              <label className="cursor-pointer flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-blue-300/35 bg-black/40 p-6 hover:border-[#63a8ee] transition">
                ...
                <div className="text-center">
                  <p className="font-bold text-white">
                    Click to upload affiliate slider image
                  </p>
              ...
              {form.preview && (
                <div className="mt-5 rounded-3xl border border-blue-300/20 bg-black/40 p-3">
                  <img
                    src={form.preview}
                    alt="preview"
                    className="w-full h-[160px] md:h-[260px] object-cover rounded-2xl"
                  />
                </div>
              )}
            </div>
                ...
                {sliders.map((item) => (
                  ...
                  >
                    <img
                      ...
                      alt={item.title || "affiliate slider"}
                      className="w-full h-[160px] md:h-[220px] object-cover rounded-2xl border border-blue-300/15"
                    />

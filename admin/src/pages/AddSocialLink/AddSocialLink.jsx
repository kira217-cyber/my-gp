...
const AddSocialLink = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        const res = await api.put(`/api/social-link/admin/${editingId}`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        toast.success(res?.data?.message || "Social link updated successfully");
      } else {
        const res = await api.post("/api/social-link/admin", payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    const ok = window.confirm("Are you sure you want to delete this social link?");
    if (!ok) return;
  ...
  const handleQuickToggle = async (item) => {
    try {
      ...
      const res = await api.put(`/api/social-link/admin/${item._id}`, payload, {
        headers: { "Content-Type": "multipart/form-data" },
      });
